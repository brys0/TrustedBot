...
public class ColorChooserCommand extends TrustedCommand {

	private final EventWaiter waiter;

	public ColorChooserCommand(EventWaiter waiter) {
	...
	protected void exec(CommandEvent e) {
		ButtonMenu menu = new ButtonMenu.Builder().setChoices("\u2764", "\ud83d\udd36", "\ud83d\udc9b", "\ud83d\udc9a", "\ud83d\udc99", "\ud83d\udc9c", "\ud83d\udda4", "\u274C").setUsers(e.getAuthor()).setText("Choose a color for your name, "+e.getMember().getAsMention()).setEventWaiter(this.waiter).setTimeout(5, TimeUnit.MINUTES).setAction(action -> {
			...
			action.getGuild().getController().removeRolesFromMember(action.getMember(), action.getMember().getRoles().stream().filter(rol -> colors.contains(rol.getName())).collect(Collectors.toList())).queue();
			if(color != 0xFFFFFF) {
				if (action.getGuild().getRolesByName(roleName, false).size() == 0) {
					action.getGuild().getController().createRole().setName(roleName).setColor(color).queue(r -> {
						action.getGuild().getController().modifyRolePositions().selectPosition(r).moveTo(roles - 2).queue(v -> {
							action.getGuild().getController().addSingleRoleToMember(action.getMember(), r).queue();
						});
					});
				} else {
					action.getGuild().getController().addSingleRoleToMember(action.getMember(), action.getGuild().getRolesByName(roleName, false).get(0)).queue();
				}
			}
